...
def positions_rugbymen_player(placement_order, graphic):
    ...
    while i < n_rugbymen:
        ...
        if color == Color.RED and (
            pos[1] > Constants.number_of_columns // 2 or pos[1] == 0
        ):  # Red characters should be placed on the left
            cond_RED = True
            while cond_RED:
                print(
                    "The position isn't correct, the red team is suppose to be on the left of the field"
                )
        ...
        if color == Color.BLUE and (
            pos[1] <= Constants.number_of_columns // 2 + 1
            or pos[1] == Constants.number_of_columns + 1
        ):  # Blue characters should be placed on the right
            cond_Blue = True
            while cond_Blue:
                print(
                ...
                if (
                    pos[1] > Constants.number_of_columns // 2 + 1
                    and pos[1] != Constants.number_of_columns + 1
                ):
                    cond_Blue = False
...
def positions_rugbymen_player_blue_AI(placement_order, graphic):
    ...
    if color == Color.BLUE:
        placement_order[Noms[0]].set_pos_x(Constants.number_of_rows // 2)
        placement_order[Noms[0]].set_pos_y(Constants.number_of_columns // 2 + 3)
        R[0] = placement_order[Noms[0]]
        graphic.display_rugbyman(R[0])

        placement_order[Noms[1]].set_pos_x(Constants.number_of_rows // 2 + 1)
        placement_order[Noms[1]].set_pos_y(Constants.number_of_columns // 2 + 3)
        R[1] = placement_order[Noms[1]]
        graphic.display_rugbyman(R[1])

        placement_order[Noms[2]].set_pos_x(Constants.number_of_rows // 2)
        placement_order[Noms[2]].set_pos_y(Constants.number_of_columns // 2 + 2)
        R[2] = placement_order[Noms[2]]
        graphic.display_rugbyman(R[2])

        placement_order[Noms[3]].set_pos_x(Constants.number_of_rows // 2 + 1)
        placement_order[Noms[3]].set_pos_y(Constants.number_of_columns // 2 + 2)
        R[3] = placement_order[Noms[3]]
        graphic.display_rugbyman(R[3])

        placement_order[Noms[4]].set_pos_x(Constants.number_of_rows // 2 - 1)
        placement_order[Noms[4]].set_pos_y(Constants.number_of_columns // 2 + 2)
        R[4] = placement_order[Noms[4]]
        graphic.display_rugbyman(R[4])

        placement_order[Noms[5]].set_pos_x(Constants.number_of_rows // 2 + 2)
        placement_order[Noms[5]].set_pos_y(Constants.number_of_columns // 2 + 2)
        R[5] = placement_order[Noms[5]]
        graphic.display_rugbyman(R[5])
    ...
    for j in range((Constants.number_of_rows + 2)):
        for i in range((Constants.number_of_columns + 2)):
            full_hitbox.append(
                pygame.Rect(
                    (
                        Constants.edge_width_normalized
                        + i * Constants.square_width_normalized
                    )
                    * screen.get_width(),
                    (
                        Constants.edge_height_normalized
                        + j * Constants.square_height_normalized
                    )
                    * screen.get_height(),
