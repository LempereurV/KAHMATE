...
class Minimax:
    ...
    def minimax_red_player(self, depth, alpha, beta, moves, possible_moves):
            ...
            for i_2 in range(i_1 + 1, 3):
                ...
                for j_1 in range(len(actions1)):
                    if depth == self.depth and not actions1[j_1][-1]:
                        ...
                        rugbyman_defender = self.game.which_rugbyman_in_pos(
                            [actions1[j_1][0], actions1[j_1][1]]
                        )

                        # If the defender is in position of tackling he will do it no matter the outcome of the tackle
                        self.actions_minimax.action_rugbyman_AI(
                            rugbyman1, rugbyman_defender, actions1
                        )
                        ...
                        for i_2_p in range(i_1 + 1, 3):
                            ...
                            for j_2 in range(len(actions2)):
                                if (
                                    ...
                                    and not self.actions_minimax.action_rugbyman_AI(
                                        rugbyman2, rugbyman_defender, actions2
                                    )
                                ):
                                    ...
                                    if [actions1[j_1][0], actions1[j_1][1]] == [
                                        actions2[j_2][0],
                                        actions2[j_2][1],
                                    ]:
                                        continue
                                    self.actions.move_rugbyman(
                                        actions2[j_2][:2], rugbyman2, actions2[j_2][2]
                                    )
                                    ...
                                    self.actions_minimax.undo_move_rugbyman(
                                        former_pos_rugbyman_2,
                                        former_ball_pos_1,
                                        rugbyman2,
                                        moove_points2,
                                    )
                    ...
                    self.actions.move_rugbyman(
                        actions1[j_1][0:2], rugbyman1, actions1[j_1][2]
                    )
                    ...
                    if rugbyman1.has_ball():
                        ...
                        for possible_throw in self.actions.available_pass():
                            if (
                                self.actions_minimax.make_pass_AI(possible_throw)
                                == False
                            ):
                                self.actions_minimax.undo_pass_AI(
                                    former_ball_pos_2, rugbyman1
                                )
                                continue
                            else:
                                for j_2 in range(len(actions2)):
                                    ...
                                    self.actions.move_rugbyman(
                                        actions2[j_2][0:2], rugbyman2, actions2[j_2][2]
                                    )
        ...
        self.actions_minimax.undo_move_rugbyman(
            former_pos_rugbyman_1, former_ball_pos_1, rugbyman1, moove_points1
        )
            ...
            for i_2 in range(i_1 + 1, 3):
                ...
                for j_1 in range(len(actions1)):
                    if depth == self.depth and not actions1[j_1][-1]:
                        ...
                        possible_moves = self.game.every_possible_move(
                            self.game.get_player_blue()
                        )

                        for i_2_p in range(len(possible_moves)):

                            rugbyman2 = possible_moves[i_2_p][0]
                            if rugbyman1 == rugbyman2:
                                continue
                            ...
                            for j_2 in range(len(actions2)):
                                if depth == self.depth and not actions2[j_2][-1]:
                                    rugbyman_defender = self.game.which_rugbyman_in_pos(
                                        [actions2[j_2][0], actions2[j_2][1]]
                                    )
                                    self.actions_minimax.action_rugbyman_AI(
                                        rugbyman2, rugbyman_defender, actions2
                                    )

                                    # the game has already taken into account the actions of the ai
                                    moves[0] = False
                                    moves[1] = False
                                    moves[2] = None
                                    return 0
