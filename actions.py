...
class Action:
    ...
    def attacker_win_charge(self, rugbyman_attacker, rugbyman_defender, possible_moves):
        ...
        self.game.get_ball().set_pos(
            rugbyman_defender.get_pos()
        )  # A bit useless here since the ball was already in the defender position
    ...
    def defender_win_charge(self, rugbyman_attacker, rugbyman_defender, possible_moves):
        ...
        if tools.norm(rugbyman_attacker.get_pos(), rugbyman_defender.get_pos()) > 1:
            self.attacker_far_from_defender(
                rugbyman_attacker, rugbyman_defender, possible_moves
            )
        return rugbyman_attacker

    def attacker_far_from_defender(
        self, rugbyman_attacker, rugbyman_defender, possible_moves
    ):
        min_norm = 100
        ...
        for move in possible_moves:
            ...
            if tools.norm(move[:2], rugbyman_defender.get_pos()) == 1:
                ...
                if tools.norm(move[:2], rugbyman_attacker.get_pos()) < min_norm:
                    ...
                    min_norm = tools.norm(
                        [move[0], move[1]], rugbyman_attacker.get_pos()
                    )
    ...
    def red_defender_win_charge(self, rugbyman_attacker, rugbyman_defender):
        if rugbyman_attacker.get_pos_y() > 1:
            ...
            self.game.get_ball().set_pos(
                [rugbyman_attacker.get_pos_x(), rugbyman_attacker.get_pos_y() - 1]
            )
        else:
            ...
            if rugbyman_attacker.get_pos_x() > 1:
                self.game.get_ball().set_pos(
                    [rugbyman_attacker.get_pos_x() - 1, rugbyman_attacker.get_pos_y()]
                )
            else:
                self.game.get_ball().set_pos(
                    [rugbyman_attacker.get_pos_x() + 1, rugbyman_attacker.get_pos_y()]
    ...
    def charging(self, rugbyman_attacker, rugbyman_defender, possible_moves):
        ...
        if (
            rugbyman_attacker.get_moves_left()
            - tools.norm(rugbyman_attacker.get_pos(), rugbyman_defender.get_pos())
            >= 1
        ):
            ...
            if (
                c_attacker + rugbyman_attacker.get_attack_bonus()
                > c_defender + rugbyman_defender.get_defense_bonus()
            ):
                self.attacker_win_charge(
                    rugbyman_attacker, rugbyman_defender, possible_moves
                )
            else:
                self.defender_win_charge(
                    rugbyman_attacker, rugbyman_defender, possible_moves
                )
    ...
    def red_choose_card(self, player, is_card_returned, pos, active_cards):
        ...
        while is_card_returned:  # the following should be reconstructed better
            ...
            while (
                pos[1] >= (Constants.number_of_columns + 2) // 2
                or pos[0] < 2
                or pos[0] > Constants.number_of_rows - 1
            ):
    ...
    def tackling(self, rugbyman_attacker, rugbyman_defender, possible_moves):
        ...
        if self.game.is_rugbyman_on_ball() == rugbyman_defender:
            ...
            if (
                ...
                self.attacker_wins_tackle(
                    rugbyman_attacker, rugbyman_defender, c_attacker, c_defender
                )
            else:
                rugbyman_attacker.set_KO()
    ...
    def attacker_wins_tackle(
        self, rugbyman_attacker, rugbyman_defender, c_attacker, c_defender
    ):
        rugbyman_defender.set_KO()
        rugbyman_defender.set_possesion(False)
        ...
        if (
            c_attacker + rugbyman_attacker.get_attack_bonus()
            > c_defender + rugbyman_defender.get_defense_bonus() + 1
        ):
            ...
            if rugbyman_defender.get_color() == Color.RED:
                if rugbyman_defender.get_pos_y() > 0:
                    self.game.get_ball().set_pos(
                        [
                            rugbyman_defender.get_pos_x(),
                            rugbyman_defender.get_pos_y() - 1,
                        ]
                    )
                else:
                    if rugbyman_defender.get_pos_x() > 0:
                        self.game.get_ball().set_pos(
                            [
                                rugbyman_defender.get_pos_x() - 1,
                                rugbyman_defender.get_pos_y(),
                            ]
                        )
                    else:
                        self.game.get_ball().set_pos(
            ...
            if possible_moves[i][3]:
                ...
                if self.game.is_rugbyman_on_ball() == rugbyman:
                    return self.charging(
                        rugbyman, self.game.which_rugbyman_in_pos(pos), possible_moves
                    )
    ...
    def make_pass(self, possible_passes):
        ...
        if pos in possible_passes:
            ...
            if (
                former_owner.get_color() is Color.RED
            ...
            if (
                former_owner.get_color() is Color.BLUE
                and pos[1] > former_owner.get_pos_y()
            ):
    ...
    def red_backward_pass_catch(self, pos, former_owner):
        ...
        for rugbyman in self.game.get_player_blue().get_rugbymen():
            ...
            if (
                rugbyman.get_pos_y() <= former_owner.get_pos_y()
                ...
                if (rugbyman.get_pos_x() - former_owner.get_pos_x()) * (
                    pos[0] - former_owner.get_pos_x()
                ) >= 0:

                    # We check if the rugbyman is closer to the ball than the former owner
                    if tools.norm(
                        rugbyman.get_pos(), former_owner.get_pos()
                    ) < tools.norm(former_owner.get_pos(), pos) and min > tools.norm(
                        rugbyman.get_pos(), former_owner.get_pos()
                    ):
...
class ActionMiniMax(Action):
    ...
    def action_rugbyman_AI_defending(self, rugbyman, possible_moves):
        ...
        if pos in possible_moves_without_scope_and_bool:
            ...
            if possible_moves[i][3]:
                ...
                if self.game.is_rugbyman_on_ball() == rugbyman:
                    return self.charging_AI_blue_defending(
                        rugbyman, self.game.which_rugbyman_in_pos(pos), possible_moves
                    )
                elif self.game.get_ball().get_pos() == pos:
                    return self.tackling_AI_defending(
                        rugbyman, self.game.which_rugbyman_in_pos(pos), possible_moves
                    )
                else:
                    print("You can only tackle the rugbyman with the ball")
    ...
    def charging_AI_blue_defending(
        self, rugbyman_attacker, rugbyman_defender, possible_moves
    ):
        """
        This function resolves the charging action between two rugbyman.
    ...
    def make_pass_AI(self, pos):
        ...
        if (
            self.game.which_rugbyman_in_pos(pos) != False
            and self.game.which_rugbyman_in_pos(pos).get_color()
            != former_owner.get_color()
        ):
            return False

        # If the pass is backward (meaning it can be catched)
        if former_owner.get_color() is Color.RED and pos[1] < former_owner.get_pos_y():
            ...
            for rugbyman in self.game.get_player_blue().get_rugbymen():
                ...
                if (
                    ...
                    if (rugbyman.get_pos_x() - former_owner.get_pos_x()) * (
                        ...
                        # We check if the rugbyman is closer to the ball than the former owner
                        if tools.norm(rugbyman.get_pos(), pos) < tools.norm(
                            former_owner.get_pos(), pos
                        ) and min > tools.norm(
                            rugbyman.get_pos(), former_owner.get_pos()
                        ):
                            return False
        # Same with the blue the only difference is that the receiver has to be above the thrower
        if former_owner.get_color() is Color.BLUE and pos[1] > former_owner.get_pos_y():
            min = 100
            for rugbyman in self.game.get_player_red().get_rugbymen():
                if (
                    rugbyman.get_pos_y() > former_owner.get_pos_y()
        ...
        if self.game.is_rugbyman_on_ball() == rugbyman_defender:
            ...
            if tools.norm(rugbyman_attacker.get_pos(), rugbyman_defender.get_pos()) > 1:
                ...
                for moves in possible_moves:
                    if (
                        tools.norm([moves[0], moves[1]], rugbyman_defender.get_pos())
                        == 1
                    ):
                        if (
                            tools.norm(
                                [moves[0], moves[1]], rugbyman_attacker.get_pos()
                            )
                            < min_norm
                            and moves[3]
                        ):  # moves[3] ensure that the square is free
...
class ActionBot(Action):
    ...
    def action_rugbyman_bot_defending(self, rugbyman, possible_moves):
        ...
        if pos in possible_moves_without_scope_and_bool:
            ...
            if possible_moves[i][3]:
                ...
                if self.game.is_rugbyman_on_ball() == rugbyman:
                    return self.charging_bot_red_defending(
                        rugbyman, self.game.which_rugbyman_in_pos(pos), possible_moves
                    )
                elif self.game.get_ball().get_pos() == pos:
                    return self.tackling_bot_red_defending(
                        rugbyman, self.game.which_rugbyman_in_pos(pos), possible_moves
                    )
                else:
                    print("You can only tackle the rugbyman with the ball")
    ...
    def action_rugbyman_bot(self, pos, rugbyman):
        ...
        if pos in possible_moves_without_scope_and_bool:
            ...
            if possible_moves[i][3]:
                ...
                if self.game.is_rugbyman_on_ball() == rugbyman:
                    return self.charging_bot(
                        rugbyman, self.game.which_rugbyman_in_pos(pos), possible_moves
                    )
                elif self.game.get_ball().get_pos() == pos:
                    return self.tackling_bot(
                        rugbyman, self.game.which_rugbyman_in_pos(pos), possible_moves
                    )
                else:
                    return False
    ...
    def charging_bot(self, rugbyman_attacker, rugbyman_defender, possible_moves):
        ...
        if (
            ...
            if (
                ...
                self.graphique.highlight_move(possible_moves)
                # This function forces the rugbyman to move
                self.move_rugbyman_after_succesfull_charging(
                    rugbyman_attacker, self.game.get_ball(), possible_moves
                )
                ...
                if rugbyman_attacker.get_color() == Color.RED:
                    if rugbyman_attacker.get_pos_y() > 1:
                        ...
                        self.game.get_ball().set_pos(
                            [
                                rugbyman_attacker.get_pos_x(),
                                rugbyman_attacker.get_pos_y() - 1,
                            ]
                        )
                    else:
                        ...
                        if rugbyman_attacker.get_pos_x() > 1:
                            self.game.get_ball().set_pos(
                                rugbyman_attacker.get_pos() + [-1, 0]
                            )
                        else:
                            self.game.get_ball().set_pos(
                                [
                                    rugbyman_attacker.get_pos_x() + 1,
                                    rugbyman_attacker.get_pos_y(),
                                ]
                            )
                else:
                    # Same but here the "behind" blue is towards the right
                ...
                if (
                    ...
                    for move in possible_moves:
                        ...
                        if tools.norm(move[:2], rugbyman_defender.get_pos()) == 1:
                            # The closest square is the natural choice
                            if (
                                tools.norm(move[:2], rugbyman_attacker.get_pos())
                                < min_norm
                            ):
    ...
    def action_rugbyman_bot_red_defending(self, rugbyman, possible_moves):
        """
        This function resolves an action that can be tackling, charging or simply moving depending on the situation
    ...
    def charging_bot_red_defending(
        self, rugbyman_attacker, rugbyman_defender, possible_moves
    ):
        """
        This function resolves the charging action between two rugbyman.
    ...
    def tackling_bot_red_defending(
        self, rugbyman_attacker, rugbyman_defender, possible_moves
    ):
