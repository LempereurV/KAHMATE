        ...
        while active_player.get_can_play():
            ...
            if rugbyman_or_ball_or_bool in active_player.get_rugbymen():
                ...
                if rugbyman_or_ball_or_bool in active_player.get_chosen_rugbymen():
                    ...
                    rugbyman_or_ball_or_bool = (
                        kahmate_actions_bot.action_rugbyman_bot_defending(
...
def compute_reward(game):
    R = np.zeros(
        (
            constants.Constants.number_of_rows + 1,
            constants.Constants.number_of_columns + 1,
        )
    )
    A = (
        game.tour_joueurs()
    )  # A matrix indicating +1 for friendly players (in red) and -1 for enemies.
    ...
    if S == 1:
        for i in range(constants.Constants.number_of_rows + 1):
            for j in range(y, constants.Constants.number_of_columns + 1):
                ...
                R[i][j] = 3 * np.exp(
                    2 * (j - y) / constants.Constants.number_of_columns + 1
                )
                ...
                if Rugbyman.color == color.Color.RED:
                    ...
                    for k in range(Rugbyman.move_points):
                        for l in range(Rugbyman.move_points):
                            if (
                                x + k < constants.Constants.number_of_rows
                                and y + l < constants.Constants.number_of_columns + 1
                            ):
                                R[x + k][y + l] -= 0.05 * np.exp(
                                    ((k) ** 2)
                                    / (constants.Constants.number_of_rows + 1)
                                ) + 0.05 * np.exp(
                                    ((l) ** 2)
    ...
    if S == -1:
        for i in range(constants.Constants.number_of_rows + 1):
            for j in range(constants.Constants.number_of_columns + 1):
                ...
                R[i][j] = np.exp(
                    -((j - y) ** 2) / (constants.Constants.number_of_columns + 1)
                )
                R[i][j] += np.exp(
                    -((i - x) ** 2) / (constants.Constants.number_of_rows + 1)
                )
...
def compute_action(game, graph, actions_bot):
    ...
    if S == 1:
        ...
        Rugbymen = sorted(
            Rugbymen, key=lambda Rugbyman: Rugbyman.get_score(), reverse=True
        )
        # This list is sorted in descending order of the highest possible reward for all rugbymen
        if Poss.get_KO() == 0:
            if (
                Poss != Rugbymen[0] and Poss != Rugbymen[1]
            ):  # if the ballkeeper can't attain the highest rewards, we will try to pass it
    ...
    if S == 0:  # no one has the ball
        ...
        for Rugbyman in game.rugbymen():
            ...
            if (
                np.any(np.all(ez_coords == np.array([x, y]), axis=1))
    ...
    if S == -1:  # the enemy has the ball
        ...
        for Rugbyman in game.rugbymen():
            ...
            if (
                ...
                if (
                    Rugbyman.get_KO() == 0
                ):  # If the rugby player has successfully tackled.
                    Poss = Rugbyman
                    a = True
                break
        Rugbymen = []
