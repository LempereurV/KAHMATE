...
class Game:
    ...
    def __init__(self, graphic, AI=False):
        ...
        self._player_red = players.Player(
            color.Color.RED, self, self._whose_turn, graphic
        )

        self._player_blue = players.Player(
            color.Color.BLUE, self, self._whose_turn, graphic, AI
        )
    ...
    def tour_joueurs(self):
        ...
        # utilisé pour l'ordi
        A = np.zeros(
            (constants.Constants.number_of_rows, constants.Constants.number_of_columns)
        )
    ...
    def is_position_correct(self, x, y):
        ...
        return (
            x >= 1
            and x <= Constants.number_of_rows
            and y >= 0
            and y <= Constants.number_of_columns + 1
        )
    ...
    def rugbymen(self):
        return players.Player.get_rugbymen(
            self.get_player_red()
        ) + players.Player.get_rugbymen(self.get_player_blue())
    ...
    def what_is_in_pos(self, Graph):
        ...
        if (
            self.which_rugbyman_in_pos(pos) in self.get_player_turn().get_rugbymen()
            and cond
    ...
    def available_move_position_recursif(
        self, rugbyman, pos, scope, color, cond, moves_dictionnary
    ):
        """
        ...
        if scope >= 0:
            ...
            if tuple(pos) in moves_dictionnary:
                moves_dictionnary[tuple(pos)] = [
                    max(moves_dictionnary[tuple(pos)][0], scope),
                    cond,
                ]
            else:
            ...
            if pos[0] + 1 <= Constants.number_of_rows:
                ...
                if self.is_square_empty(new_pos):
                    self.available_move_position_recursif(
                        rugbyman, new_pos, scope - 1, color, True, moves_dictionnary
                    )
                elif (
                    self.which_rugbyman_in_pos(new_pos) != False
                    and self.which_rugbyman_in_pos(new_pos).get_color() != color
                    and (
                        self.which_rugbyman_in_pos(new_pos).has_ball()
                        or rugbyman.has_ball()
                    )
                ):
                    if scope > 0:
                        if tuple(new_pos) in moves_dictionnary:
                            moves_dictionnary[tuple(new_pos)] = [
                                max(moves_dictionnary[tuple(new_pos)][0], scope - 1),
                                False,
                            ]
                        else:
    ...
    def available_move_position(self, rugbyman):
        ...
        Liste_treated = [
            ...
            for key, value in moves_dictionnary.items()
            if (
                key[0] != rugbyman,
                rugbyman.get_pos_x() and key[1] != rugbyman.get_pos_y(),
            )
        ]
    ...
    def is_game_over(self):
        ...
        for rugbyman in self.rugbymen():
            if (
                ...
                return True
            if (
                rugbyman.get_pos_y() == Constants.number_of_columns + 1
    ...
    def reward_function(self, player):
        ...
        reward += (
            self.get_ball().get_pos_y() - (Constants.number_of_columns + 1) // 2
        ) * 20
        ...
        if player.get_color() == color.Color.RED:
            for rugbyman in player.get_rugbymen():
                ...
                reward += (
                    -tools.norm(rugbyman.get_pos(), self.get_ball().get_pos()) * 10
                )
    ...
    def every_possible_move(self, player):
        ...
        for rugbyman in player.get_rugbymen():
            if (
                rugbyman.get_move_points() == rugbyman.get_moves_left()
                and rugbyman.get_KO() == 0
            ):
                R = [[rugbyman] + self.available_move_position(rugbyman)] + R
        return sorted(
            R, key=lambda x: tools.norm(x[0].get_pos(), self.get_ball().get_pos())
        )
